...
const text_to_translate = "Item In Cart";
const prev_key = "top_to_rate"
const key_to_add = "item_in_cart"
...
for(let i = 0; i < key_to_add.length; i++) {
...
for (let i=0; i<file_paths.length; i++) {
    ...
    // let idx = file_data;
    let id1 = file_data.indexOf('announcement');

    let id2 = file_data.indexOf(prev_key);

    id1 = file_data.indexOf("\n", id1);

    id2 = file_data.indexOf("\n", id2);

    const prev_text = file_data.substring(0, id1);

    const after_text = file_data.substring(id2 + 1);
    ...
    // .then(response => {
        ...
        const final_text = prev_text + after_text;
