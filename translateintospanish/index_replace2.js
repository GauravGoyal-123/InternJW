...
for (let obj of keys3) {
  for(const [key, value] of Object.entries(obj)) {

    const text_to_translate = value;
    const prev_key = key;
    // const key_to_add = "view_all_order"
    ...
    for (let i=0; i<file_paths.length; i++) {
        ...
        let second_occ;
        if(idx !== -1) {
          second_occ = file_data.indexOf(prev_key, idx + 1);
        }
        if(second_occ == -1) continue;
    
        second_occ = file_data.indexOf(":", second_occ);
        let idx2 = file_data.indexOf("\n", second_occ);
    
        const prev_text = file_data.substring(0, second_occ + 1);
